...
export function toPanelAssistantLive(slideIdx: number, live: AssistantLiveState): ChatAssistantLive {
...
export type CopilotLockedMode = 'cowork' | 'modify'

export function useCopilotForSlide(
  module: ModuleName,
  slideMeta: SlideMeta,
  lockedMode: CopilotLockedMode,
) {
  ...
  const modeForUi: ChatMode = lockedMode
  ...
  return {
    ...
    handleCoworkFill,
    handleWebSearch,
    clearSlideChat: () => clearSlideChat(slideIdx),
    clearWebSearch: () => clearWebSearch(slideIdx),
  }
}
